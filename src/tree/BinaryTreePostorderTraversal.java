...
public class BinaryTreePostorderTraversal {
    ...
    public List<Integer> postorderTraversal(TreeNode root) {
        ...
        while (!stack.empty()) {
            node = stack.peek();
            if (prev == null || prev.left == node || prev.right == node) { // traverse down the tree
                if (node.left != null) {
                    // 找到最左孩子
            ...
            } else if (node.left == prev) { // traverse up the tree from the left
                if (node.right != null) {
                    stack.push(node.right);
                }
    ...
    public List<Integer> postorderTraversalTwoStack(TreeNode root) {
        ...
        s1.push(root);
        while (!s1.empty()) {
            TreeNode node_node = s1.pop();
            if (node_node.left != null)
                s1.push(node_node.left);
            if (node_node.right != null)
                s1.push(node_node.right);
